...
- **Trigger:** The workflow is triggered when a pull request receives the `fast-forward` label (`pull_request` event, type `labeled`).
- **Conditions:**
  - The event must be a pull request being labelled.
  - The label must be `fast-forward`.
